...
class GUMP
{
    ...
    public function validate_some_text($field, $input, $param = null)
    {
        ...
        if (!preg_match("/^([\w,\.!\?-_\s])+$/iu", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
     ...
     * @return mixed
     */
    public function validate_utf8_alpha($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }

        if (!preg_match("/^(\p{L})+$/iu", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
     ...
     * @return mixed
     */
    public function validate_utf8_alpha_spase_dash($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }

        if (!preg_match("/^([\p{L}-\s])+$/iu", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
     ...
     * @return mixed
     */
    public function validate_phone_number_ext($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }

        if (!preg_match("/^([\d-\(\)+])+$/iu", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
     ...
     * @return mixed
     */
    public function validate_address_ext($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }

        if (!preg_match("/^([\w\s-+,\.\/№#])+$/iu", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
